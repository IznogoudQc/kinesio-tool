...
test('categoryToScore — échelle 0-4 (Excellent = 4, comme l’ancien logiciel)', () => {
  assert.equal(categoryToScore('A_AMELIORER'), 0)
  assert.equal(categoryToScore('ACCEPTABLE'), 1)
  assert.equal(categoryToScore('BIEN'), 2)
  assert.equal(categoryToScore('TRES_BIEN'), 3)
  assert.equal(categoryToScore('EXCELLENT'), 4)
  assert.equal(categoryToScore(null), null)
})

test('scoreToCategory — bornes 0.5 / 1.5 / 2.5 / 3.5 (échelle 0-4)', () => {
  assert.equal(scoreToCategory(0.0), 'A_AMELIORER')
  assert.equal(scoreToCategory(0.49), 'A_AMELIORER')
  assert.equal(scoreToCategory(0.5), 'ACCEPTABLE')
  assert.equal(scoreToCategory(1.49), 'ACCEPTABLE')
  assert.equal(scoreToCategory(1.5), 'BIEN')
  assert.equal(scoreToCategory(2.49), 'BIEN')
  assert.equal(scoreToCategory(2.5), 'TRES_BIEN')
  assert.equal(scoreToCategory(3.49), 'TRES_BIEN')
  assert.equal(scoreToCategory(3.5), 'EXCELLENT')
  assert.equal(scoreToCategory(4.0), 'EXCELLENT')

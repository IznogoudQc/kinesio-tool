...
test('categoryToScore — barème CSEP 0.5-4.5', () => {
  assert.equal(categoryToScore('A_AMELIORER'), 0.5)
  assert.equal(categoryToScore('ACCEPTABLE'), 1.5)
  assert.equal(categoryToScore('BIEN'), 2.5)
  assert.equal(categoryToScore('TRES_BIEN'), 3.5)
  assert.equal(categoryToScore('EXCELLENT'), 4.5)
  assert.equal(categoryToScore(null), null)
})

test('scoreToCategory — bornes 1.0 / 2.0 / 3.0 / 4.0', () => {
  assert.equal(scoreToCategory(0.5), 'A_AMELIORER')
  assert.equal(scoreToCategory(0.99), 'A_AMELIORER')
  assert.equal(scoreToCategory(1.0), 'ACCEPTABLE')
  assert.equal(scoreToCategory(1.5), 'ACCEPTABLE')
  assert.equal(scoreToCategory(2.0), 'BIEN')
  assert.equal(scoreToCategory(2.5), 'BIEN')
  assert.equal(scoreToCategory(3.0), 'TRES_BIEN')

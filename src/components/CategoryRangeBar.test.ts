 ...
 *
 * Positions ALIGNÉES sur les segments (P10/P25/P50/P75 → 20/40/60/80 %), pour que
 * le marqueur tombe toujours dans le même segment que la catégorie affichée.
 *  - VO2max 49 H 48y (≥ p75) → ~97 % (segment Excellent)
 *  - % gras 30.2 H 48y (lowerIsBetter, Acceptable) → ~39 % (segment Acceptable)
...
test('VO2max 49 H 48y → ~97 % (dans Excellent)', () => {
  const pos = calculatePosition(49, VO2MAX_M40)
  assert.ok(close(pos, 97, 2), `attendu ~97, reçu ${pos}`)
})

test('VO2max exactement à P50 (35) → 60 % (jonction Bien/Très bien)', () => {
  assert.equal(calculatePosition(35, VO2MAX_M40), 60)
})
...
test('% gras 30.2 H 48y (lowerIsBetter) → segment Acceptable (20-40 %)', () => {
  const pos = calculatePosition(30.2, BODY_FAT_M40, true)
  // 30.2 tombe entre p25 (30) et p10 (35) — en raison de lowerIsBetter,
  // c'est dans la zone Acceptable (20-40 %), près de la jonction avec Bien.
  assert.ok(pos >= 20 && pos <= 40, `attendu 20-40, reçu ${pos}`)
})

test('% gras 14 (P90 — très lean) → 100 % (Excellent)', () => {
...
test('Push-ups 28 H 48y (M 40-49 percentiles ≈ 9/12/16/22/28) → 100 % (P90)', () => {
